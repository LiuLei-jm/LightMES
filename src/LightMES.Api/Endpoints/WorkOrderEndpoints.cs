...
public static class WorkOrderEndpoints
{
    public static void MapWorkOrderEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group
            .MapPost(
                "/",
                async ([FromBody] CreateWorkOrderCommand command, ISender sender) =>
                {
        ...
        group
            .MapPost(
                "/sync-erp",
                async ([FromBody] SyncErpWorkOrderCommand command, ISender sender) =>
                {
                    var success = await sender.Send(command);
                    return success
                        ? Results.Ok("ERP工单同步成功")
                        : Results.BadRequest("同步失败，请检查工艺路线及防重");
                }
            )
        ...
        group
            .MapPut(
                "/{id:guid}",
                async (Guid id, [FromBody] UpdateWorkOrderCommand command, ISender sender) =>
                {
                    if (id != command.Id)
                        return Results.BadRequest("路由 ID 与请求体 ID 不一致");
                    var result = await sender.Send(command);
