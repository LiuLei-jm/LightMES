...
public class CreateEquipmentCommandValidator : AbstractValidator<CreateEquipmentCommand>
{
    ...
    public CreateEquipmentCommandValidator(IAppDbContext context)
    {
        ...
        RuleFor(v => v.EquipmentCode)
            ...
            .MaximumLength(50)
            .WithMessage("设备编码最多50个字符");
        RuleFor(v => v.EquipmentName)
            .NotEmpty()
            .WithMessage("设备名称不能为空.")
            .MaximumLength(100)
            .WithMessage("设备名称最多100个字符.");
        When(
            x => !string.IsNullOrWhiteSpace(x.EquipmentCode),
            () =>
            {
                RuleFor(v => v.EquipmentCode)
    ...
    {
        return !await _context.Equipments.AnyAsync(x => x.EquipmentCode == code, token);
    }
}
