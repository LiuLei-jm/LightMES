...
{
    public static async Task SeedDefaultDataAsync(AppDbContext context, IPasswordHasher passwordHasher)
    {
        if (!context.Roles.Any())
        {
            var adminRole = new Role(SystemRoles.AdminId, SystemRoles.AdminName, "系统超级管理员");
            var supervisorRole = new Role(SystemRoles.SupervisorId, SystemRoles.SupervisorName, "车间班组长");
            var operatorRole = new Role(SystemRoles.OperatorId, SystemRoles.OperatorName, "一线操作工");
            var qcRole = new Role(SystemRoles.QCId, SystemRoles.QCName, "质检员");
            var plannerRole = new Role(SystemRoles.PlannerId, SystemRoles.PlannerName, "生产计划员");
            ...
            {
                adminPermissions.Add(new RolePermission { RoleId = SystemRoles.AdminId, Permission = permission });
            }
            ...
            var userRoleRelation = new UserRole
            {
